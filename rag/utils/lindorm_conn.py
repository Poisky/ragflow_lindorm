...
logger = logging.getLogger('ragflow.lindorm_conn')

@singleton
class LindormConnection(DocStoreConnection):
    def __init__(self):
        self.info = {}
        logger.info(f"Use Lindorm {settings.Lindorm['hosts']} as the doc engine.")
        for _ in range(ATTEMPT_TIME):
            try:
                ...
                logger.warning(f"{str(e)}. Waiting Lindorm {settings.Lindorm['hosts']} to be healthy.")
                time.sleep(5)
        if not self.client.ping():
            msg = f"Lindorm {settings.Lindorm['hosts']} is unhealthy in 120s."
            logger.error(msg)
            raise Exception(msg)
        v = self.info.get("version", {"number": "2.18.0"})
        v = v["number"].split(".")[0]
        if int(v) < 2:
            msg = f"Lindorm version must be greater than or equal to 2, current version: {v}"
            logger.error(msg)
            raise Exception(msg)
        fp_mapping = os.path.join(get_project_base_directory(), "conf", "lindorm_mapping.json")
        if not os.path.exists(fp_mapping):
            msg = f"Lindorm mapping file not found at {fp_mapping}"
            logger.error(msg)
            raise Exception(msg)
        self.mapping = json.load(open(fp_mapping, "r"))
        logger.info(f"Lindorm {settings.Lindorm['hosts']} is healthy.")
    ...
    def health(self) -> dict:
        health_dict = dict(self.client.cluster.health())
        health_dict["type"] = "lindorm"
        return health_dict
    ...
    def createIdx(self, indexName: str, knowledgebaseId: str, vectorSize: int):
        ...
        try:
            ...
            return IndicesClient(self.client).create(index=indexName,
                                                 body=self.mapping)
        except Exception:
            logger.exception("LindormConnection.createIndex error %s" % (indexName))
    ...
    def deleteIdx(self, indexName: str, knowledgebaseId: str):
        if len(knowledgebaseId) > 0:
            ...
            return
        try:
            self.client.indices.delete(index=indexName, allow_no_indices=True)
        except NotFoundError:
            pass
        except Exception:
            logger.exception("LindormConnection.deleteIdx error %s" % (indexName))

    def indexExist(self, indexName: str, knowledgebaseId: str = None) -> bool:
        s = Index(indexName, self.client)
        for i in range(ATTEMPT_TIME):
            try:
                return s.exists()
            except Exception as e:
                logger.exception("LindormConnection.indexExist got exception")
    ...
    def search(
        ...
        for m in matchExprs:
            if isinstance(m, MatchTextExpr) and abs(vector_similarity_weight - 1.0) > 1e-6:
                ...
                q_vector_dict = {
                    ...
                    "from": offset,
                    "_source": True,
                    "ext": {"lvector": {"filter_type": "efficient_filter"}}
                }
        ...
        if q_vector_dict and abs(vector_similarity_weight - 1.0) < 1e-6:
            for i in range(ATTEMPT_TIME):
                try:
                    print("vector_search", q_vector_dict)
            ...
            for i in range(ATTEMPT_TIME):
                try:
                    ...
                    res = self.client.transport.perform_request(
                        ...
                        body='\n'.join([json.dumps(line) for line in msearch_body]) + '\n',
                        timeout=600
                    )
                    #print("msearch response", str(res))
    ...
    def get(self, chunkId: str, indexName: str, knowledgebaseIds: list[str]) -> dict | None:
        ...
        for i in range(ATTEMPT_TIME):
            try:
                res = self.client.get(index=(indexName),
                ...
                logger.exception(f"LindormConnection.get({chunkId}) got exception")
                if str(e).find("Timeout") > 0:
                    continue
                raise e
        logger.error("LindormConnection.get timeout for 3 times!")
        raise Exception("LindormConnection.get timeout.")

    def insert(self, documents: list[dict], indexName: str, knowledgebaseId: str = None) -> list[str]:
        # Refers to https://opensearch.org/docs/latest/api-reference/document-apis/bulk/
        print("LindormConnection.insert")
        operations = []
        for d in documents:
            assert "_id" not in d
            assert "id" in d
        ...
        for _ in range(ATTEMPT_TIME):
            try:
                res = []
                r = self.client.bulk(index=(indexName), body=operations,
    ...
    def update(self, condition: dict, newValue: dict, indexName: str, knowledgebaseId: str) -> bool:
        print("LindormConnection.update")
        ...
        if "id" in condition and isinstance(condition["id"], str):
            ...
            for i in range(ATTEMPT_TIME):
                try:
                    self.client.update(index=indexName, id=chunkId, body=doc)
                    return True
                except Exception as e:
                    logger.exception(
                        f"LindormConnection.update(index={indexName}, id={id}, doc={json.dumps(condition, ensure_ascii=False)}) got exception")
        ...
        for _ in range(ATTEMPT_TIME):
            try:
                ...
                logger.error("LindormConnection.update got exception: " + str(e) + "\n".join(scripts))
    ...
    def delete(self, condition: dict, indexName: str, knowledgebaseId: str) -> int:
        print("LindormConnection.delete")
        qry = None
        assert "_id" not in condition
        if "id" in condition:
        ...
        logger.debug("LindormConnection.delete query: " + json.dumps(qry.to_dict()))
        for _ in range(ATTEMPT_TIME):
            try:
                ...
                logger.warning("LindormConnection.delete got exception: " + str(e))
                if re.search(r"(timeout|connection)", str(e).lower()):
                    time.sleep(3)
                    continue
    ...
    def sql(self, sql: str, fetch_size: int, format: str):
        ...
        for i in range(ATTEMPT_TIME):
            try:
                res = self.client.sql.query(body={"query": sql, "fetch_size": fetch_size}, format=format,
                                        request_timeout="2s")
                return res
